...
public class CommProcessor extends CommandManager implements IParsedPacketListener, IFilterListener
{
		...
		switch ( pack.getCmd() )
		{
			...
			case CMD.A_DotData:
				...
				if ( isPrevDotDown )
				{
					...
					this.processDot( sectionId, ownerId, noteId, pageId, timeLong, X, Y, FLOAT_X, FLOAT_Y, FORCE, DotType.toIntInkDot( DotType.PEN_ACTION_DOWN ), currColor );
				}
				else
				{
					// 펜업이 아닌 경우 미들 도트로 저장
					this.processDot( sectionId, ownerId, noteId, pageId, timeLong, X, Y, FLOAT_X, FLOAT_Y, FORCE, DotType.toIntInkDot( DotType.PEN_ACTION_MOVE ), currColor );
				}
			...
			case CMD.A_DotUpDownData:
				...
				else if ( PEN_UP_DOWN == 1 )
				{
					if ( prevPacket != null )
					{
						...
						this.processDot( sectionId, ownerId, noteId, pageId, pTimeLong, pX, pY, pFX, pFY, pFORCE, DotType.toIntInkDot( DotType.PEN_ACTION_UP ), currColor );
					}
			...
			case CMD.A_PenStatusResponse:
				...
				{
					JSONObject job = new JSONObject()
					.put( JsonTag.STRING_PROTOCOL_VERSION, stat_version )
	...
	{
		Fdot tempFdot = new Fdot();
		tempFdot.setDot( type, sectionId, ownerId, noteId, pageId, x, y, timeLong, fx, fy, force, color );
	...
	{
		ShowTextCommand command = new ShowTextCommand( CMD.P_ShowText, this );
		command.setStatus( showString24 );
		execute( command );
	}
