...
public class OfflineFileParser implements IFilterListener
{
	 ...
	 * 
	 * @param filepath
	 *            location of offline data file
	 * @param filename
	 *            name of offline data file
	 */
	 ...
	 * 
	 * @param filepath
	 *            The location of the data files are stored offline
	 * 
	 * @return
	 */
	...
	private void parseBody() throws Exception
	{
		...
		{	
			...
			else
			{
				...
				Fdot dot = new Fdot();
				dot.sectionId = this.sectionId;
				dot.ownerId = this.ownerId;
				dot.noteId = this.noteId;
				dot.pageId = this.pageId;

				long time = (int) (body[idx] & 0xFF);

				dot.x = ByteConverter.byteArrayToShort( Packet.copyOfRange( body, idx + 1, 2 ) );
				dot.y = ByteConverter.byteArrayToShort( Packet.copyOfRange( body, idx + 3, 2 ) );

				dot.fx = (int) (body[idx + 5] & 0xFF);
				dot.fy = (int) (body[idx + 6] & 0xFF);

				dot.force = (int) (body[idx + 7] & 0xFF);

				dot.color = lhLineColor;

				boolean isPenUp = false;
				
				if ( dotCount == 1 )
				{
					dot.dotType = DotType.toIntInkDot( DotType.PEN_ACTION_DOWN );
					dot.timestamp = lhPenDnTime + time;
					prevTimestamp = dot.timestamp;
				}
				else if ( lhDotTotal > dotCount )
				{
					dot.dotType = DotType.toIntInkDot( DotType.PEN_ACTION_MOVE );
					dot.timestamp = prevTimestamp + time;
					prevTimestamp = dot.timestamp;
				}
				else
				{
					dot.dotType = DotType.toIntInkDot( DotType.PEN_ACTION_UP );
					dot.timestamp = lhPenUpTime;
					isPenUp = true;
				}
